...
class LFM:
    ...
    #随机梯度下降
    def Gradient_descent(self,rcd_train,U,VT,alpha,lambda_u,lambda_i):
        m,k = U.shape
        k,n = VT.shape
        ...
        for l in range(100):
            ...
            for i in range(c-1):
                ...
                for j in range(k):
                    ...
                    Ui[int(rcd_train[0,i]),j] = U[int(rcd_train[0,i]),j] + alpha * (e*VT[j,int(rcd_train[1,i])] - lambda_u*U[int(rcd_train[0,i]),j])
                    VTi[j,int(rcd_train[1,i])] = VT[j,int(rcd_train[1,i])] + alpha * (e*U[int(rcd_train[0,i]),j]  - lambda_i*VT[j,int(rcd_train[1,i])])
    ...
    def test(self):
        ...
        self.lfm_num = int(math.pow(c_max,0.33))
        ...
        U0,Sigma,VT = randomized_svd(sparse_matrix_train_mean,n_components = lfm_num)
        U = U0*Sigma
        # print('U')
        # print(U)
        # print('VT')
        ...
        #矩阵拓展引入用户偏差和物品偏差
        #用户偏差拓展
        Up = self.getUp(U,user_deviation)
        VTp = self.getVTp(VT,item_deviation)
        print(Up)
        print('*'*60)
        print(VTp)
        ...
        alpha = 0.0001
        lambda_u,lambda_i = 0.25,0.25
        U,VT = self.Gradient_descent(rcd_train,Up,VTp,alpha,lambda_u,lambda_i)
        Rate = np.dot(U,VT)
        sum0 = 0
        for i in range(len(rcd_train)):
