...
class LFM:
    ...
    def test(self):
        ...
        self.lfm_num = int(math.pow(c_max,0.33))  #设置隐变量的个数
        ...
        #矩阵拓展引入用户偏差和物品偏差
        #用户偏差拓展
        Up0 = self.getUp(U,user_deviation)
        VTp0 = self.getVTp(VT,item_deviation)
        # print(Up)
        # print('*'*60)
        # print(VTp)
        ...
        self.Up,self.VTp = self.Gradient_descent(rcd_train,Up0,VTp0,alpha,lambda_u,lambda_i)
        Rate = np.dot(U,VT)
        sum0 = 0
        for i in range(len(rcd_train)):
