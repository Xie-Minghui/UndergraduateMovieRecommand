...
def Cosine_similarity(inA:np.mat,inB:np.mat,item1:int,item2:int,rating_denominator,denominatorA,denominatorB,rating_nominator)->float: #调整余弦
    '''
        传入参数：
...
def Choose_dimension(Sigma:np.array,info_remain_percent:float)->int:
    ...
    for i in range(0,Len):
        ...
        if sum_now/totle_sum >= info_remain_percent:
            ...
            break
    return ans + 1
def Calculate_items(origin_data:np.mat, mean_centered_data_transf:np.mat,Similarity_calculate_means,rating_denominator,denominatorA,denominatorB,rating_nominator)->np.mat:
    '''
    ...
    for i in range(0,c):
        ...
        for j in range(i+1,c):
            ...
            # if i == 1 and j == 4:
            #     print('nihao',overlap)
            similarity_item_matrix[i,j] = similarity_item_matrix[j,i] = Similarity_calculate_means(mean_centered_data_transf[:,i],mean_centered_data_transf[:,j],i,j,rating_denominator,denominatorA,denominatorB,rating_nominator)
    return similarity_item_matrix

def Calculate_items_similarty(origin_data:np.mat,mean_centered_data:np.mat,rating_denominator,denominatorA,denominatorB,rating_nominator)->np.mat:
    '''
        传入参数：
    ...
    # print("nihao1")
    similarity_item_matrix = Calculate_items(origin_data,mean_centered_data_transf,Cosine_similarity,rating_denominator,denominatorA,denominatorB,rating_nominator)#Pearson_similarity
    return similarity_item_matrix
...
def test(user,k=5,predict_num=3):
    ...
    similarity_item_matrix=Calculate_items_similarty(origin_data,mean_centered_data,rating_denominator,denominatorA,denominatorB,rating_nominator)
    #--------------------------------------------------------------------------------------
    print("nihao")
    top_k_item,top_k_score,recommend_reasons_items = ItemRecommend(origin_data,similarity_item_matrix,user,k,predict_num)
